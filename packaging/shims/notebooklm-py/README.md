...
**This project is now published as [`gemini-notebook-py`](https://pypi.org/project/gemini-notebook-py/).**

Google renamed NotebookLM to Gemini Notebook, so the distribution name followed. This package is kept as a permanent forwarding shim: it ships no code of its own and installs `gemini-notebook-py` for you.
...
Every extra is mirrored, so `pip install "notebooklm-py[mcp]"` installs exactly what
...
- `uvx --from "notebooklm-py[mcp]" notebooklm-mcp` — works.
- `pipx install notebooklm-py` — add `--include-deps` to get the commands, or
  install `gemini-notebook-py` directly (recommended).

## Upgrading from a pre-0.9 install

`notebooklm-py <= 0.8` shipped the `notebooklm` package files directly and collides
