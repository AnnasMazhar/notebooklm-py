...
the P2.1 notebook/source reads and P6.3 plain-note CRUD. Both list/get slices and all five
`NotesAPI` operations now delegate through semantic services and that client-owned backend. The
inert P2.2 core adds notebook create/title-update/delete
...
four notebook/source read handlers and five note handlers remain active catalogued authorities.
The note-backed mind-map/artifact service remains an explicitly named legacy RPC consumer until
the MIND_MAP_* slice, so it never borrows NOTE_* authority. This bounded
...
| `_web/backend.py` | Web semantic backend over the existing `RpcExecutor`; 12 executable handlers cover P2.1/P2.2 plus P6.3 plain notes, while the staged P2.3 URL-source composite remains facade-inert. Notebook/source reads and note CRUD are active; notebook mutations and URL registration remain inert. |
| `_web/registry.py` | Closed web disposition registry over every `Operation`: 12 executable typed handlers, one explicitly staged P2.3 URL handler rejected by production dispatch, and an unsupported disposition for every other operation. |
