...
| Artifact generation | `ArtifactsAPI` delegates report and video kickoff to the backend-neutral `ReportFamilyService` / `VideoFamilyService` (`_studio/documents.py`); their web handlers use the established `_artifact/payloads.py` builders and preserve the current `CREATE_ARTIFACT` wire requests. Unmigrated families, `revise_slide`, and `retry_failed` still use `ArtifactGenerationService` (`_artifact/generation.py`). `ArtifactPollingService` owns leader/follower polling with `operation_scope(...)` and a feature-local `PollRegistry`; `ArtifactsAPI` registers a close-time drain hook for poll cleanup. |
...
| `_records.py` | Frozen, slotted, protocol-neutral input/output records and `OperationDef` values for P2 notebook/source operations, P5.1/P5.4 Studio catalog and report/video generation, and P6.3 plain-note CRUD, including the closed URL-source error evidence. |
...
| `_studio/` | Private transport-neutral Studio services: the P5.1 heterogeneous catalog/classifier and the P5.4 report/video family boundary. |
| `_web/backend.py` | Web semantic backend over the existing `RpcExecutor`; 17 active handlers cover P2 notebook/source operations, P5.1 Studio catalog reads, P5.4 report/video generation, and P6.3 plain-note CRUD. |
...
├── _records.py                  # Neutral P2/P5.1/P5.4/P6.3 backend DTOs and operation definitions
├── _studio/                     # Transport-neutral Studio services
│   ├── catalog.py               # Heterogeneous catalog and family classifier (P5.1)
