...
| [`_auth/profile_store.py`](../src/notebooklm/_auth/profile_store.py) | Path-owned synchronous document/session reads, blocking cookie transactions, and typed in-band account read/update/clear with their distinct corruption and bounded-lock policies. Owns no cache, baseline, or migration scheduler; replacement/token policy remains deferred. |
| [`_auth/storage.py`](../src/notebooklm/_auth/storage.py) | Shrinking v0.x persistence policy/compatibility facade. Retains raw account adapters, legacy reconciliation/promotion/sibling scrub, lock wrappers, cookie adapters, and replacement policy; delegates cookie/account transactions to `ProfileStore` and imports transaction functions as exact aliases. Empty account mappings remain absent for promotion while non-empty unknown mappings remain present. |
...
| [`_auth/account.py`](../src/notebooklm/_auth/account.py) | **Account NETWORK identity**: probing `?authuser=N`, extracting the active email, formatting the wire value, and `repair_account_metadata_from_playwright_storage`. Typed in-band account read/update/clear lives in `ProfileStore`; raw compatibility, legacy reconciliation/promotion, and sibling scrub remain in `_auth/storage.py`. |
...
| `_auth/profile_store.py` | Path-owned fresh document/session/account reads, bounded account update/clear, and blocking cookie transactions over immutable decisions (ADR-0034) |
...
│   ├── profile_store.py         # Path-owned reads + cookie/account transaction policies (ADR-0034)
...
│   ├── storage.py               # v0.x persistence policy/compat facade over ProfileStore + typed commit spine
