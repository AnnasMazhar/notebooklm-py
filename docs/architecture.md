...
the P2.1 notebook/source reads, P2.2 notebook mutations, P2.3 URL/YouTube registration, and P6.3
plain-note CRUD. These public paths delegate through transport-neutral semantic services and that
client-owned backend. The URL-source handler owns ordinary and hidden YouTube dispatch, exact
pre-create reconciliation, and best-effort title mutation under one absolute deadline. P3's first
live codec routes GET_SOURCE structured documents through the strict web codec boundary. The
...
notebook/source read handlers, three notebook-mutation handlers, the URL-source composite, and five
plain-note handlers are active catalogued authorities. The note-backed mind-map/artifact service
remains an explicitly named legacy RPC consumer until the MIND_MAP_* slice, so it never borrows
NOTE_* authority. This bounded classification is mutation-tested and shrinks as later slices
activate handlers.
...
| `_records.py` | Frozen, slotted, protocol-neutral input/output records and `OperationDef` values for the P2.1 notebook/source reads, P2.2 notebook mutations, and P2.3 URL-source mutation receipt plus closed serializable public-error compatibility evidence. |
...
| `_projectors.py` | Shared compatibility projectors from neutral notebook/source/note records to the existing public `Notebook` / `Source` / `Note` models, using their normal constructors and no wire adapters. |
...
| `_web/backend.py` | Web semantic backend over the existing `RpcExecutor`; 13 active handlers cover P2 notebook/source reads and mutations, P2 URL registration, and P6.3 plain-note CRUD. |
| `_web/registry.py` | Closed web disposition registry over every `Operation`: 13 executable typed handlers and an unsupported disposition for every other operation. |
...
├── _projectors.py               # Neutral record-to-public Notebook/Source/Note compatibility projectors
...
├── _records.py                  # Neutral P2/P6.3 backend DTOs and operation definitions
