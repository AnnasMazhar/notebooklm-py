...
| [`_auth/storage.py`](../src/notebooklm/_auth/storage.py) | Shrinking v0.x persistence policy/compatibility facade. Retains raw account adapters, legacy reconciliation/promotion/sibling scrub, lock wrappers, cookie adapters, the minted live-jar snapshot adapter, and token policy. Remint, login, and minted callables are thin typed `ProfileStore` adapters; minted alone translates the private owner refusal outside its handler to context-free canonical `MasterTokenError`, and login alone performs post-APPLIED legacy reconciliation after lock release. Filter names and transaction functions remain exact compatibility aliases. |
