...
│   ├── source_capacity.py       # Transport-neutral SourceCapacity snapshot + pre-add quota guard: pairs explicit SourceCounts with the live source_limit and rejects before create at the boundary (#1962)
...
│   ├── source_listing.py        # Click-free `source list` fetch core: label-filter resolution plus notebook-wide SourceCapacity snapshots shared by MCP/REST
