...
P6.3 plain-note CRUD, and P6.4 source-label/collection operations.
...
that same ledger; P5.1 through P5.6 and P6.3/P6.4 extend it to all 36 current handlers. The
...
handlers, and eleven source-label/collection handlers are active catalogued authorities.
`MindMapsAPI` and the retained note-backed listing/download seams remain explicitly named legacy
RPC consumers until the MIND_MAP_* slice, so they never borrow NOTE_* authority. This bounded
...
| Baseline | Freezes |
| --- | --- |
| `operation_catalog` | 86 operations with 168 exact authority rows (43 multi-authority); 56 native rows (17 multi-site, four honestly `not_recorded` goldens) with variant-specific evidence and per-binding override proof; the exact 36-operation active web policy/native-idempotency ledger; 146 namespace methods (eight local-only), ten root-client members, and 11 divergences (10 authority, one policy) |
...
| `_web/backend.py` | Web semantic backend over the existing `RpcExecutor`; 36 active handlers cover P2 notebook/source operations, P5.1 Studio catalog reads, P5.2–P5.6 Studio families/Drive export, P6.3 plain-note CRUD, and P6.4 source-label/collection operations. |
