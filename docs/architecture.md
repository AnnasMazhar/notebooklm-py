...
P6.2 Research, and P6.3 plain-note CRUD.
...
that same ledger; P5.1 through P5.6 and P6.2/P6.3 extend it to all 29 current handlers. The remaining phase
...
handlers, plus four Research wire handlers, are active catalogued authorities. `MindMapsAPI` and the retained note-backed
...
`research.wait` and `research.import_verify` deliberately remain unsupported at the web port.
Neither is a wire operation: the wait loop composes `research.poll` under its own cadence and
deadline, while verified import composes `research.import` with the `source.list` snapshots and
probes used for reconciliation. Keeping those policies in `ResearchService` prevents polling and
idempotency policy from leaking behind the protocol seam.

P0 adds four ADR-0022 contract baselines before runtime delegation:

| Baseline | Freezes |
| --- | --- |
| `operation_catalog` | 86 operations with 164 exact authority rows (41 multi-authority); 56 native rows (19 multi-site, four honestly `not_recorded` goldens) with variant-specific evidence and per-binding override proof; the exact 29-operation active web policy/native-idempotency ledger; 146 namespace methods (eight local-only), ten root-client members, and 11 divergences (10 authority, one policy) |
...
| `_records.py` | Frozen, slotted, protocol-neutral input/output records and `OperationDef` values for P2 notebook/source operations, P5.1 Studio catalog, P5.2 Audio, P5.3 Quiz/Flashcards, P5.4 Report/Video, P5.5 Infographic/Slide Deck, P5.6 Data Table/Mind Map/Drive export, P6.2 Research, and P6.3 plain-note CRUD, plus P3 notebook-guide/source/artifact/label/collection/sharing decode values and closed URL-source error evidence. |
...
| `_studio/` | Private transport-neutral Studio boundary: the P5.1 heterogeneous catalog/classifier plus P5.2 Audio, P5.3 Quiz/Flashcards, P5.4 Report/Video, P5.5 Infographic/Slide Deck, and P5.6 Data Table/Mind Map/Drive-export services. |
...
| `_web/backend.py` | Web semantic backend over the existing `RpcExecutor`; 29 active handlers cover P2 notebook/source operations, P5.1 Studio catalog reads, P5.2–P5.6 Studio families/Drive export, P6.2 Research, and P6.3 plain-note CRUD. |
| `_web/policy.py` | Exact P4 ledger, extended to all 29 active web workflows: semantic policy, every reachable native method/variant, reviewed native idempotency, and optional reported divergence. It audits parity but never controls retry execution. |
| `_web/registry.py` | Closed web disposition registry over every `Operation`: 29 executable typed handlers and an unsupported disposition for every other operation, including the two service-composed Research operations. |
...
| `_web/codec/` | P3/P6.2 web response ownership: notebook, source, artifact, label, collection, sharing, report/guide, and Research codecs return frozen neutral records; `documents.py` alone returns the approved exported `StructuredDocument` value exemption. Codec bindings are tied to cassette-backed golden families and never call public parsing factories. |
...
| `_research.py` | `client.research` compatibility facade over the transport-neutral P6.2 `ResearchService`; it retains its RPC collaborator only to construct the default source lister used by import verification. |
...
├── _research_service.py         # Transport-neutral research start/poll/wait/cancel/import service (P6.2)
├── _records.py                  # Neutral P2/P5.1-P5.6/P6.2-P6.3 DTOs + P3 decoded resource records
...
│   ├── backend.py               # RpcExecutor-backed P2/P5.1-P5.6/P6.2-P6.3 handlers
...
├── _research.py                 # ResearchAPI compatibility facade over ResearchService (P6.2)
├── _notes.py                    # NotesAPI
