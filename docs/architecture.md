...
| [`_auth/master_token.py`](../src/notebooklm/_auth/master_token.py) | Headless master-token auth: its raw reader projects the captured legacy dict from one `MasterTokenFile` sample, and its writer preserves the call-time `storage.write_master_token` seam. It also retains minting primitives and the whole audited transaction (`bootstrap_from_oauth_token`, `remint_from_stored_token`, internal `bootstrap_storage_from_master_token`, bool-collapsing `bootstrap_missing_storage_from_master_token`, and `assert_account_writable`). |
