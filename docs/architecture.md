...
| [`_auth/storage.py`](../src/notebooklm/_auth/storage.py) | Shrinking v0.x persistence policy/compatibility facade. Retains raw signatures/results, lock and cookie adapters, the minted live-jar snapshot adapter, and token policy. Account resolution/promotion/scrub and login/account reconciliation delegate to the concrete `profile_migration` owners; remint/login/minted callables remain thin typed `ProfileStore` adapters. Filter names and transaction functions remain exact compatibility aliases. |
...
| [`_auth/cookies.py`](../src/notebooklm/_auth/cookies.py) | Cookie maps, `_update_cookie_input`, and the one-sample paired loader. Each successfully converted live cookie supplies identity/value/runtime fields while SameSite comes from that same sanitized raw row, preserving first-successful duplicate behavior in the typed baseline. |
...
| [`_auth/recovery.py`](../src/notebooklm/_auth/recovery.py) | Client-neutral L3/L4 recovery adapters and exact paired replacement provenance; successful headless/master/cold reloads carry the typed baseline from the same raw sample as the replacement live jar. Consumes the `single_flight` core and keeps the per-loop revalidate-on-bump epoch (`_COLD_SUCCESS_GENERATIONS`) and mutation lock as consumer-side policy. |
...
| [`_auth/account.py`](../src/notebooklm/_auth/account.py) | **Account NETWORK identity**: probing `?authuser=N`, extracting the active email, formatting the wire value, and `repair_account_metadata_from_playwright_storage`. Typed in-band account read/update/clear lives in `ProfileStore`; legacy file policy and lifecycle live in `_auth/profile_migration.py`; raw compatibility remains in `_auth/storage.py`. |
...
| [`_auth/refresh.py`](../src/notebooklm/_auth/refresh.py) | Token refresh driver (external login command, secret redaction) and exact-baseline ladder adapter. It resolves the account route at every token attempt/replacement, returns the baseline paired with the successful initial, refresh-command, headless, or master-token jar, coalesces via `single_flight`, and serializes the subprocess with the per-path flock. The frozen `RefreshDeps` record retains late-bound runner/flock/path injection; the route callback remains optional for v0.x callers. |
...
The measured persistence boundary is 1,150 lines in `storage.py`, 311 in
`profile_migration.py`, 794 in `profile_store.py`, and 96 in `cookie_filter.py`: 2,351 lines total.
The migration module is internal composition, not a public `ProfileStore` extension surface.
The Phase 9 loader owners measure 816 lines in `tokens.py`, 1,195 in `refresh.py`, and 996 in
`client.py`. The client currently projects the typed file baseline through the existing
`AuthTokens.cookie_snapshot` constructor bridge; direct `CookiePersistence` store/baseline
registration is explicitly deferred to Phase 10.
...
| `_auth/profile_store.py` | Path-owned fresh document/session/account reads, account update/clear, blocking cookie transactions, and bounded remint/login/minted replacement; minted owns latest-owner/filter/rebind/commit (ADR-0034) |
