...
- Cookie adapters: public `save_cookies_to_storage`, `snapshot_cookie_jar`, and
  `advance_cookie_snapshot_after_save` remain. `CookiePersistence.__init__` and
  `CookiePersistence.capture_open_snapshot` retain their compatibility roles; the private
  `_save_v0_callback` isolates an explicitly injected `cookie_saver=`. The old module-identity
  fallback and private `refresh.save_cookies_to_storage` alias are retired.
