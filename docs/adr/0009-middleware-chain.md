...
| `max_response_bytes` | `int \| None` | `RuntimeTransport.perform_authed_post` (seeded only when a per-request response-size cap is supplied — currently the chat path's `chat_max_response_bytes`; absent otherwise so metadata RPCs use `MAX_RPC_RESPONSE_BYTES`) | `RuntimeTransport.terminal` (forwards to `Kernel.post(max_response_bytes=...)`, which passes it to the streamed-response size guard) |
