...
|---|---|---|---|
| `GET_SOURCE` fulltext | `_source/content.py` passes the body to `_row_adapters.documents.build_document`, producing `SourceFulltext.document` | `TestSourcesGoldenDecoded::test_get_fulltext_decoded_golden` pins the RPC result while `test_citation_alignment.py` pins the captured tree, UTF-16 offsets, and readable rendering | A P3 source codec may construct the exempt graph directly through its public constructors. |
