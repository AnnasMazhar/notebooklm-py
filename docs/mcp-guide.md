...
Source selection and accounting are explicit across the source-consuming tools.
Unfiltered `source_list` returns notebook-wide `source_counts`: `records_total` includes
id-less phantoms and duplicate rows, while `by_status.ready` is the confirmed
processed bucket (`by_status.unknown` identifies omitted/unmapped statuses).
Its `status` filter also accepts `unspecified` and `pending_deletion`.
`chat_ask`, `suggest_prompts`, and `studio_generate` accept either `source_ids`
or `source_filter={"statuses": [...], "types": [...]}` (never both). Values
within one axis are ORed and the two axes are ANDed; omitting both preserves the
historical all-addressable-source behavior.
