...
- **A backend-processed failed source add can leave a ghost row.** Adapter-driven
  adds now compare the notebook's `quota_counted` sources with the live
  `source_limit` before the create RPC, so a quota-boundary rejection creates no
