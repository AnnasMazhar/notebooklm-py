...
- **Retrying a partial import used to fail — now it's idempotent.** The raw `IMPORT_RESEARCH`
  ...
  that error. Pass `allow_duplicate=true` to re-add them anyway. Fixed by
