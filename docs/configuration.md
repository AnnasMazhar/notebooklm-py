...
| `NOTEBOOKLM_MAX_CHAT_RESPONSE_BYTES` | Maximum buffered response body size for `client.chat.ask`. Must be a positive integer. Use when very large notebooks exceed the chat default because the streamed response includes notebook sync/state frames as well as answer text. Explicit `NotebookLMClient(chat_max_response_bytes=...)` wins over this env var. | `268435456` |
...
| `NOTEBOOKLM_MAX_CHAT_RESPONSE_BYTES` | Maximum buffered response body size for `client.chat.ask`. Use a positive integer when a large notebook's streamed chat response exceeds the 256 MiB chat default because it includes notebook sync/state frames as well as answer text. Boolean-like strings (`true`/`false`/`yes`/`no`/`on`/`off`) are rejected instead of coerced. | Explicit `NotebookLMClient(chat_max_response_bytes=...)` → env var → `268435456` | `_env.get_max_chat_response_bytes_override` + `_runtime.config.normalize_chat_max_response_bytes` |
