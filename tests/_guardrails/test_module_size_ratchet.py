...
ALLOWLISTED_CEILINGS: dict[str, int] = {
    ...
    "exceptions.py": 1599,
    # sanctioned merge (ADR-0033) — PR 1.1 of the `_auth` consolidation: the
    # persistence seam that was spelled as three cap-split files becomes one deep
    # module. ``_auth/storage_writer.py`` (981) and ``_auth/storage_transaction.py``
    # (183) were absorbed in full and reduced to one-line re-export shims in the
    # same PR; the merged module carries the lock primitives, the bounded acquire,
