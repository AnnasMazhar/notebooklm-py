...
def test_client_core_exposes_cookie_persistence(tmp_path: Path) -> None:
    ...
    core._backend._cookie_persistence.loaded_cookie_snapshot = baseline

    assert isinstance(core._backend._cookie_persistence, CookiePersistence)
    assert core._backend._cookie_persistence.loaded_cookie_snapshot is baseline
    # The save lock is a stock ``threading.Lock`` owned by the collaborator.
    assert isinstance(core._backend._cookie_persistence.save_lock, type(threading.Lock()))
...
async def test_client_core_save_cookies_routes_through_injected_seam_and_to_thread(
    ...
    await core._backend._lifecycle.save_cookies(core._backend._cookie_persistence, _jar())
