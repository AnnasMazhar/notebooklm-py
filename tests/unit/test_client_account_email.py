...
through pytest-httpx by installing a real ``httpx.AsyncClient`` on the provider
kernel (the seam ``client._provider._kernel.get_http_client()`` reads) so mocked page GETs
...
from notebooklm._runtime import web_cookie_provider as web_cookie_provider_module
...
def _install_probe_client(client: NotebookLMClient) -> httpx.AsyncClient:
    ...
    install_http_client_for_test(client._provider._kernel, http_client)
    return http_client
...
async def test_in_memory_email_returned_without_http(httpx_mock: HTTPXMock) -> None:
    ...
    assert client._provider._account_email_cache == "alice@example.com"
...
async def test_post_close_persisted_email_uses_retained_kernel_jar(
    ...
    await client._provider._kernel.aclose()
...
async def test_live_probe_restarts_after_profile_session_replacement(
    ...
    monkeypatch.setattr(web_cookie_provider_module, "_probe_authuser", blocked_probe)
...
async def test_live_probe_does_not_self_heal_into_previously_advanced_profile(
    ...
    monkeypatch.setattr(web_cookie_provider_module, "_probe_authuser", probe_old_session)
    try:
        assert await client.get_account_email() == "old@example.com"
    finally:
        await http_client.aclose()
...
async def test_persisted_email_from_a_different_live_session_is_ignored(tmp_path) -> None:
    ...
    assert await client.get_account_email(live_fallback=False) is None
    assert client._provider._account_email_cache is None
